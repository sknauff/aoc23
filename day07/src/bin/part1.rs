...
impl Hand {
    ...
    fn get_hand_type(&self) -> HandType {
        let unique_faces = self.get_face_counts();
        let mut face_counts = unique_faces.values().cloned().collect::<Vec<u64>>();
        face_counts.sort();

        if face_counts[0] == 5 {
            HandType::FiveOfAKind
        } else if face_counts[1] == 4 {
            HandType::FourOfAKind
        } else if face_counts[1] == 3 && face_counts[0] == 2 {
            HandType::FullHouse
        } else if face_counts.last().unwrap().to_owned() == 3 {
            HandType::ThreeOfAKind
        } else if face_counts[2] == 2 && face_counts[1] == 2 {
            HandType::TwoPair
        } else if face_counts.last().unwrap().to_owned() == 2 {
            HandType::OnePair
        } else {
            HandType::HighCard
        }
